...
class ConvDicoLearnFISTANN(nn.Module):
    ...
    def __init__(self, Dyn2DEncObj, T=1, n_filters=8, kernel_size=[7, 7, 7], lambda_reg=5e-7):
        ...
        filter_shape = (
            n_filters,
            1,
        ) + tuple([kernel_size[k] for k in range(len(kernel_size))])
    ...
    def forward(self, y: torch.Tensor, csm: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        for kiter in range(self.T):
            ...
            s_new = self.soft_thresholding(z - 1.0 / op_norm_BHB * (BHBz - BHy), threshold)

            # update beta
            beta_new = (1.0 + torch.sqrt(1 + 4 * beta**2)) / 2.0

            # update sparse codes
            z = s_new + (beta - 1.0) / beta_new * (s_new - s)
...
class ConvDicoLearnFISTA(CineModel):
    def __init__(self, lr=1e-5, schedule=False):
        ...
        self.cdl_fista = ConvDicoLearnFISTANN(Dyn2DCartEncObj(), T=12, n_filters=32, kernel_size=[7, 7, 7], lambda_reg=5e-2)
