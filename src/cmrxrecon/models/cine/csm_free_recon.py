...
class LambdaCNN(nn.Module):
    ...
    def __init__(self, 
                 cnn_block=Unet(3, channels_in=2, channels_out=1, layer=1, filters=16, n_convs_per_stage=4)
                 ):
...
class CSMFreeReconNN(nn.Module):
    ...
    def __init__(self, 
                 img_unet=Unet_felix(3, channels_in=20, channels_out=20, layer=2, filters=16, conv_per_enc_block=2, conv_per_dec_block=2),
                 T=1,
                 normfactor=1e2,
                 mode = 'xyt'):
        super().__init__()
...
class CSMFreeReconDeluxeNN(nn.Module):
    ...
    def __init__(self, 
        ...
        layer, n_convs_per_stage, filters = 3, 4, 64
        pre_trained_weights_path = '/data/kofler01/projects/CMRxRecon/pre_trained_models/unet_E{}C{}K{}.pt'.format(layer, n_convs_per_stage, filters)
        self.img_cnn.load_state_dict(torch.load(pre_trained_weights_path))
    ...
    def forward(self, y: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        for ku in range(self.T):
            ...
            H = lambda x: torch.fft.ifftn(mask * torch.fft.fftn(x, dim=(-2, -1), norm="ortho"), dim=(-2, -1), norm="ortho") + t * lambda_map * x
