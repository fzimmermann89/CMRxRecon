...
class CSMFreeReconNN(nn.Module):
    ...
    def __init__(self, img_unet=Unet(3, channels_in=2, channels_out=2, layer=1, filters=32, n_convs_per_stage=4), T=1, lambda_init=1e-5, normfactor=1e2):
        super().__init__()
...
class CSMFreeRecon(CineModel):
    def __init__(self, T=1, lr=1e-3, weight_decay=0.0, schedule=False):
        super().__init__()
