...
def process(fn):
    ...
    csm = [sp_mri.app.EspiritCalib(z_slice, max_iter=MAX_ITER, thresh=THRESHOLD, show_pbar=False).run() for z_slice in k_centered[0]]
    csm = np.array(csm)
    Nc = csm.shape[1]
...
for job in ["T1map", "T2map"]:
    files.extend(list((PATH / "FullSample").rglob(f"*{job}.mat")))
Parallel(n_jobs=32, verbose=10)(delayed(process)(fn) for fn in files)
